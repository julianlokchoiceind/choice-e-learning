...
- Luôn luôn sử dụng alias import (CỰC KỲ ƯU TIÊN CHO TOÀN BỘ FILE HỆ THỐNG)

### 5. Tuân thủ Cấu trúc DDD

-Hạn chế tạo nhiều cấp thư mục lồng nhau khi không cần thiết
-Ưu tiên tổ chức code theo domain thay vì theo chức năng kỹ thuật
-Giữ độ sâu của cấu trúc thư mục không quá 3-4 cấp
-Mỗi file nên có một trách nhiệm duy nhất và rõ ràng
-Nhóm các file liên quan trong cùng một thư mục domain thay vì tạo thêm thư mục con
-File index.ts nên được sử dụng để export từ một thư mục, không nên chứa logic phức tạp
...
### 7. Hướng dẫn tổ chức thư mục theo Domain (RẤT QUAN TRỌNG)

## 7.1. Nguyên tắc cơ bản
- Tổ chức code theo domain/tính năng, không theo layer kỹ thuật
- Tuân thủ mẫu đã thiết lập trong codebase hiện tại
- Đảm bảo tính nhất quán giữa các domain

## 7.2. Cấu trúc thư mục theo domain
```
src/
├── client/components/{domain}/          # Component React cho domain
├── client/hooks/{domain}/               # Custom hooks cho domain  
├── server/services/{domain}/            # Logic nghiệp vụ
├── shared/types/{domain}/               # Types TypeScript
├── shared/schemas/{domain}/             # Schema validation
└── app/api/{domain}/                    # Endpoints API
```
## 7.3. Quy trình phát triển tính năng theo domain
### 3.1. Xác định domain
- Xác định tính năng thuộc domain nào (courses, lessons, auth...)
- Kiểm tra cấu trúc hiện tại của domain đó

### 7.3.2. Triển khai theo các bước trong tài liệu
- **Bước 1**: Định nghĩa Types trong `/shared/types/{domain}/`
- **Bước 2**: Tạo Schemas trong `/shared/schemas/{domain}/`
- **Bước 3**: Triển khai Backend trong `/server/services/{domain}/`
- **Bước 4**: Triển khai Frontend trong `/client/components/{domain}/`
- **Bước 5**: Cập nhật file index.ts

## 7.4. Lợi ích
- Dễ dàng điều hướng và tìm kiếm code
- Onboarding nhanh hơn cho developer mới
- Bảo trì và mở rộng dễ dàng hơn
- Ranh giới rõ ràng giữa các tính năng
